...
void print_rule(DICT *d, CODE c)
{
    printf("Rule %d -> ", c);
    if (is_terminal(d, d->rule[c].left)) {
        printf("\'%c\' ", (unsigned char) d->rule[c].left);
    } else {
        printf("-%d- ", d->rule[c].left);
    }
    if (is_terminal(d, d->rule[c].right)) {
        printf("\'%c\' ", (unsigned char) d->rule[c].right);
    } else {
        printf("-%d- ", d->rule[c].right);
    }
    printf("\n");
    if (!is_terminal(d, d->rule[c].left))  {
        print_rule(d, d->rule[c].left);
    }
    if (!is_terminal(d, d->rule[c].right))  {
        print_rule(d, d->rule[c].right);
    }
}
...
DICT *get_excerpt_from_grammar(DICT *d, uint from, uint to)
{
    ...
    printf("Excerpted start rule: ");
    for (uint i = 0; i < ret->seq_len; i++) {
        if (ret->comp_seq[i] < 256) {
            printf("\'%c\' ", (unsigned char) ret->comp_seq[i]);
        } else {
            printf("-%d- ", ret->comp_seq[i]);
        }
    }
    printf("\n");
    for (uint i = 0; i < ret->seq_len; i++) {
        if (ret->comp_seq[i] >= 256) {
            print_rule(d, ret->comp_seq[i]);
        }
    }
